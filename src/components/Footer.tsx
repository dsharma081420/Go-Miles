import Image from "next/image";
import Link from "next/link";

const social = [
  {
    name: "Facebook",
    href: "https://www.facebook.com/",
    icon: (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    name: "LinkedIn",
    href: "https://www.linkedin.com/",
    icon: (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
];

export function Footer() {
  return (
    <footer className="border-t border-white/10 bg-[#030712] py-14">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          <div className="max-w-sm">
            <Link href="/" className="flex items-center gap-3">
              <Image
                src="/images/logo-gomiles.png"
                alt=""
                width={44}
                height={44}
                className="h-10 w-10 object-contain"
              />
              <span className="font-semibold text-white">Go Miles Dispatch Services</span>
            </Link>
            <p className="mt-4 text-sm leading-relaxed text-slate-400">
              Professional truck dispatch for owner-operators and fleets across Canada and
              cross-border lanes. Higher miles. Stronger rates. Less stress.
            </p>
            <div className="mt-6 flex gap-4">
              {social.map((s) => (
                <a
                  key={s.name}
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-slate-400 transition hover:text-white"
                  aria-label={s.name}
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-10 sm:gap-16">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wider text-slate-500">
                Quick links
              </p>
              <ul className="mt-4 space-y-2 text-sm">
                <li>
                  <a href="#services" className="text-slate-300 hover:text-white">
                    Services
                  </a>
                </li>
                <li>
                  <a href="#pricing" className="text-slate-300 hover:text-white">
                    Pricing
                  </a>
                </li>
                <li>
                  <a href="#faq" className="text-slate-300 hover:text-white">
                    FAQ
                  </a>
                </li>
                <li>
                  <a href="#contact" className="text-slate-300 hover:text-white">
                    Contact
                  </a>
                </li>
              </ul>
            </div>
            <div>
              <p className="text-sm font-semibold uppercase tracking-wider text-slate-500">
                Trust
              </p>
              <ul className="mt-4 space-y-2 text-sm text-slate-300">
                <li>Ontario-based dispatch</li>
                <li>Transparent commission</li>
                <li>24/7 carrier support</li>
              </ul>
            </div>
          </div>
        </div>
        <div className="mt-12 flex flex-col gap-4 border-t border-white/10 pt-8 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          <p>© {new Date().getFullYear()} Go Miles Dispatch Services. All rights reserved.</p>
          <p>
            <a href="https://www.gomiles.ca" className="hover:text-slate-300">
              www.gomiles.ca
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
}
